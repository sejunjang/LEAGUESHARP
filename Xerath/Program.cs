...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("UseENear", "Use E to Enemy Near Mouse").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));


            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterFullCombo", "Draw damage after full combo").SetValue(true);
            Utility.HpBarDamageIndicator.DamageToUnit += hero => (float)Player.GetSpellDamage(hero, SpellSlot.R) * 3 + (float)Player.GetSpellDamage(hero, SpellSlot.Q) + (float)Player.GetSpellDamage(hero, SpellSlot.W) + (float)Player.GetSpellDamage(hero, SpellSlot.E);
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            //Cast E Near Mouse
            if (Config.Item("UseENear").GetValue<KeyBind>().Active && E.IsReady())
            {
                foreach (
                    var enemy in
                        ObjectManager.Get<Obj_AI_Hero>()
                            .Where(
                                enemy =>
                                    enemy.Team != Player.Team && Player.Distance(enemy, true) <= Math.Pow(E.Range, 2))
                            .Where(
                                enemy =>
                                    enemy.IsValidTarget(E.Range) && enemy.Distance(Game.CursorPos, true) <= 150 * 150))
                {
                    UseSpells(false, false, true);
                }
            }
